...
if [[ "$MODEL" = "k2p" || "$MODEL" = "k2p-32" ]] && [ $MTK_DRIVER -eq 1 ];then
	git clone --depth 1 https://github.com/caicaicai21/mt7615-dbdc-linux5.4.git mtk
	check_git
fi
...
git clone --depth 1 -b openwrt-21.02 https://github.com/caicaicai21/openwrt_custom_packages.git custom-packages
check_git
cd custom-packages
...
EOF
cp ./package/kernel/shortcut-fe/patch-files/*.patch ./target/linux/generic/hack-5.4
else
rm -f ./target/linux/generic/hack-5.4/952-net-conntrack-events-support-multiple-registrant.patch
rm -f ./target/linux/generic/hack-5.4/999-shortcut-fe-support.patch
fi
